...
class Unit:
    ...
    def getContribution(self, balance=True):
        """
        计算各顶点对面片重心的面积贡献系数。

        :param balance: 是否使用平衡模式。
            - False: 从顶点0出发扇形三角剖分（原始逻辑，偏向顶点0）
            - True:  枚举所有 C(N,3) 个三角形，按面积加权均衡分配（默认）
        :return: 各顶点贡献系数列表（长度等于顶点数，和为1）
        """
        seq_point = np.array(self.getSeqPoint())
        ...
        if not balance:
            ...
            area_tri = [
                abs(0.5 * (
                    (seq_point[i][X] - seq_point[0][X]) * (seq_point[i+1][Y] - seq_point[0][Y])
                - (seq_point[i][Y] - seq_point[0][Y]) * (seq_point[i+1][X] - seq_point[0][X])
                ))
            ...
            # ---- 平衡逻辑：枚举全部 C(N,3) 个三角形 ----
            total_area = 0.0
            for (i, j, k) in combinations(range(kp_len), 3):
                area = abs(0.5 * (
                    (seq_point[j][X] - seq_point[i][X]) * (seq_point[k][Y] - seq_point[i][Y])
                - (seq_point[j][Y] - seq_point[i][Y]) * (seq_point[k][X] - seq_point[i][X])
                ))
                c[i] += 1/3 * area
                c[j] += 1/3 * area
                c[k] += 1/3 * area
